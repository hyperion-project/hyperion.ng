...
FramebufferGrabberOptions parseFramebufferGrabberOptions(const QCoreApplication& app)
{
	Parser parser(QStringLiteral("Framebuffer capture application for Hyperion. Will automatically search a Hyperion server if -a option is not used. Please note that if you have more than one server running it's more or less random which one will be used."));
	...
	FramebufferGrabberOptions opts;
	opts.fps            = argFps.getInt(parser);
	opts.deviceIdx      = argDeviceIdx.getInt(parser);
