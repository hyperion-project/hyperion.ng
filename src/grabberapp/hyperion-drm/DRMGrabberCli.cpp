...
{
	Parser parser(QStringLiteral("DRM-Grabber capture application for Hyperion. Will automatically search a Hyperion server if -a option is not used. Please note that if you have more than one server running it's more or less random which one will be used."));

	IntOption& argDevice = parser.add<IntOption>('d', "device", "Set the DRM device index to capture (e.g., 0 for /dev/dri/card0) [default: %1]", "0");
	...
	DRMGrabberOptions opts;
	opts.fps            = argFps.getInt(parser);
	opts.deviceIdx      = argDevice.getInt(parser);
