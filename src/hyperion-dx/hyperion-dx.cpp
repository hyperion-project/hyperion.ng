...
int main(int argc, char ** argv)
{
	...
	try
	{
		...
		Parser parser("DirectX interface capture application for Hyperion");
		...
		else
		{
			...
			FlatBufferConnection flatbuf("DirectX Standalone", address, argPriority.getInt(parser), parser.isSet(argSkipReply));
	...
	catch (const std::runtime_error & e)
	{
		...
		Error(Logger::getInstance("DXGRABBER"), "%s", e.what());
		return -1;
	}
	return 0;
}
