	...
	, _log(Logger::getInstance("ComponentRegister"))
{
	...
	connect(_hyperion, &Hyperion::compStateChangeRequest, this, &ComponentRegister::compStateChangeRequest);
}
...
void ComponentRegister::setNewComponentState(const hyperion::Components comp, const bool activated)
{
	if(_componentStates[comp] != activated)
	{
		...
		// emit component has changed state
	 	emit _hyperion->updatedComponentState(comp, activated);
	}
}

void ComponentRegister::compStateChangeRequest(const hyperion::Components comp, const bool activated)
{
