...
PriorityMuxer::PriorityMuxer(int ledCount)
	: QObject(), _log(Logger::getInstance("HYPERION")), _currentPriority(PriorityMuxer::LOWEST_PRIORITY), _manualSelectedPriority(256), _activeInputs(), _lowestPriorityInfo(), _sourceAutoSelectEnabled(true), _updateTimer(new QTimer()), _timer(new QTimer()), _blockTimer(new QTimer())
{
	...
	_lowestPriorityInfo.componentId = hyperion::TYPE_COLOR;
...
void PriorityMuxer::clearAll(bool forceClearAll)
{
	...
	else
	{
		for (auto key : _activeInputs.keys())
		{
			...
			if ((info.componentId == hyperion::TYPE_COLOR || info.componentId == hyperion::TYPE_EFFECT || info.componentId == hyperion::TYPE_IMAGE) && key < PriorityMuxer::LOWEST_PRIORITY - 1)
			{
				clearInput(key);
			}
		}
...
void PriorityMuxer::setCurrentTime(void)
{
	...
	for (auto infoIt = _activeInputs.begin(); infoIt != _activeInputs.end();)
	{
		...
		else
		{
			...
			if (infoIt->priority < 254 && infoIt->timeoutTime_ms > 0 && (infoIt->componentId == hyperion::TYPE_EFFECT || infoIt->componentId == hyperion::TYPE_COLOR || infoIt->componentId == hyperion::TYPE_IMAGE))
