...
{
	qCDebug(memory_image_create).noquote() << QString("|Image| CREATE: Creating Image [%1] of size %2x%3").arg(_instanceId).arg(width).arg(height);
}
...
Image<Pixel_T>::Image(const Image& other) :
	...
	_instanceId(++_image_instance_counter)
{
	qCDebug(memory_image_copy).noquote() << QString("|Image| COPY (SHALLOW): Image handle [%1] created, sharing data with handle [%2].").arg(_instanceId).arg(other._instanceId);
}
	...
	_instanceId(src._instanceId)
{
	...
	qCDebug(memory_image_move).noquote() << QString("|Image| MOVE: Image handle [%1] has been moved into a new instance.").arg(_instanceId);
}
...
Image<Pixel_T>::~Image()
{
	...
	if (isDetached())
	{
		qCDebug(memory_image_destroy).noquote() << QString("|Image| DESTROY (HANDLE): Image handle [%1] destroyed. This was the last handle.").arg(_instanceId);
	}
	else
	{
		qCDebug(memory_image_destroy).noquote() << QString("|Image| DESTROY (HANDLE): Image handle [%1] destroyed. Other handles still exist.").arg(_instanceId);
	}
}
...
Image<Pixel_T>& Image<Pixel_T>::operator=(const Image& other)
{
	...
	{
		_d_ptr = other._d_ptr;
		qCDebug(memory_image_assign).noquote() << QString("|Image| ASSIGN (SHALLOW HANDLE): Image handle [%1] now shares data with handle [%2].").arg(_instanceId).arg(other._instanceId);
	}
	return *this;
}
...
Image<Pixel_T>& Image<Pixel_T>::operator=(Image&& other) noexcept
{
	if (this != &other)
	{
		...
		other._instanceId = 0;
		qCDebug(memory_image_assign).noquote() << QString("|Image| ASSIGN (MOVE): Image handle [%1] has taken ownership from another handle.").arg(_instanceId);
	}
	return *this;
}
...
typename Image<Pixel_T>::pixel_type* Image<Pixel_T>::memptr()
{
	if (!isDetached())
	{
		qCDebug(memory_image_copy).noquote() << QString("|Image| COPY (DEEP): memptr() on shared Image handle [%1] is causing a detach.").arg(_instanceId);
	}
	_d_ptr.detach();
