	...
	, _userEnabled(false)
{
	...
	connect(_hyperion, &Hyperion::compStateChangeRequest, this, &BlackBorderProcessor::compStateChangeRequest);
}
...
void BlackBorderProcessor::handleSettingsUpdate(const settings::type& type, const QJsonDocument& config)
{
	if(type == settings::BLACKBORDER)
	{
		...
		// eval the comp state
		_hyperion->setNewComponentState(hyperion::COMP_BLACKBORDER, obj["enable"].toBool(true));
	}
}

void BlackBorderProcessor::compStateChangeRequest(const hyperion::Components component, bool enable)
{
