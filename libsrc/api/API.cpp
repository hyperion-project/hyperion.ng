...
{
	if(!dat.args.isEmpty()){
		QMetaObject::invokeMethod(_hyperion, "setEffect", Qt::QueuedConnection
 	   , Q_ARG(QString, dat.effectName)
 	   , Q_ARG(QJsonObject, dat.args)
 	   , Q_ARG(int, dat.priority)
 	   , Q_ARG(int, dat.duration)
 	   , Q_ARG(QString, dat.pythonScript)
 	   , Q_ARG(QString, dat.origin)
 	   , Q_ARG(QString, dat.data));
   } else {
...
{
	if(!_adminAuthorized) return false;
	bool res;
