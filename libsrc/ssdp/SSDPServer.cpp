...
SSDPServer::SSDPServer(QObject *parent)
	: QObject(parent), _log(Logger::getInstance("SSDP")), _udpSocket(nullptr), _running(false)
{
}
...
{
	QString message = UPNP_MSEARCH_RESPONSE.arg(SSDP_MAX_AGE, QDateTime::currentDateTimeUtc().toString("ddd, dd MMM yyyy HH:mm:ss GMT"), _descAddress, _serverHeader, st, _uuid, _fbsPort, _jssPort, _name);
...
void SSDPServer::sendAlive(const QString &st)
{
	...
	QString message = UPNP_ALIVE_MESSAGE.arg(SSDP_MAX_AGE, _descAddress, st, _serverHeader, tempUSN, _fbsPort, _jssPort, _name);
...
{
	QString message = UPNP_UPDATE_MESSAGE.arg(_descAddress, st, _uuid + "::" + st);
